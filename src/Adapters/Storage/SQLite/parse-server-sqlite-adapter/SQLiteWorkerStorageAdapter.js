...
const deserializeWorkerError = (serializedError, logDuplicateKey, logSanitizedError) => {
  ...
  const error = serializedError?.isParseError ? new Parse.Error(properties.code, message) : new Error(message);
...
class SQLiteWorkerClient {
  constructor(options, logDuplicateKey, logSanitizedError) {
  ...
  _handleMessage(message) {
    ...
    if (message.error) {
      pendingRequest.reject(deserializeWorkerError(message.error, this._logDuplicateKey, this._logSanitizedError));
    } else {
  ...
  setAdapterProperty(property, value) {
    if (this._fatalError) {
...
const createLegacyDatabaseFacade = workerClient => ({
  collection: collectionName => ({
    updateOne: (query = {}, update = {}) => workerClient.invoke('__legacyDatabaseUpdateOne', [collectionName, query, update]),
    find: (query = {}) => ({
      toArray: () => workerClient.invoke('__legacyDatabaseFind', [collectionName, query])
    })
  })
});
const createAdaptiveCollectionFacade = (workerClient, className) => ({
  find: (query = {}) => workerClient.invoke('__adaptiveCollectionFind', [className, query]),
  _ensureSparseUniqueIndexInBackground: index => workerClient.invoke('__adaptiveCollectionEnsureSparseUniqueIndex', [className, index])
});
function createWorkerStorageAdapter(workerOptions, publicURI, adapterPrototype, releaseMainThreadResources, logDuplicateKey, logSanitizedError) {
  ...
  try {
    ...
    throw new TypeError('SQLite worker options must be structured-cloneable; pass custom providers as a module name/path');
  }
  const workerClient = new SQLiteWorkerClient(clonedOptions, logDuplicateKey, logSanitizedError);
  let shutdownPromise;
  const target = {
    _uri: publicURI,
