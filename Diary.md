...
### Dotted Multikey Indexes And Final Planner Audit
- Reproduced the remaining dotted-Object index miss: a declaration such as `code.coding.code` could query scalar JSON directly but could not index values reached through an Array below the Object root.
- Dotted paths below Object or Array roots now use one trigger-maintained typed shadow table when an index is declared. Positive equality, set, range, regex, `$all`, and `distinct` queries use native SQLite indexes; non-indexed paths keep the recursive SQL correctness fallback.
- The shadow-row builder and query fallback now share one recursive terminal walker. It handles direct scalar terminals, one or more nested Arrays, terminal Arrays, Date values, Pointer values, and generic Object values without per-row JavaScript.
- Indexed `$all` groups typed values in SQL and binds same-type sets once, so 1,200-value queries stay below SQLite's expression-depth limit. Dotted `$containedBy` and dotted `distinct` now use the same terminal traversal rules.
- Array shadow triggers are versioned. Existing databases rebuild the shadow rows once when the trigger format changes, then normal writes maintain them incrementally.
- Non-unique multikey declarations no longer retain a useless raw-JSON physical index. On 20,000-row local writes this removed roughly 10% overhead for root Arrays and 15% for nested Object multikey paths while indexed reads stayed around 0.007 ms.
- Physical scalar index names now include the class token because SQLite index names are database-global. `getIndexes()` still returns the original logical Parse index name.
- Small ordered `$in` / same-field OR merges now include native top-level Pointer and Date values, not only String, Number, and Boolean values. A broad two-Pointer compound-index benchmark improved from roughly 2.99 ms with a scan/temp sort to 0.034 ms with merged ordered seeks; selective sets remained within roughly 0.003 ms.
- Do not force ordered base-table scans for every sorted multikey query. A 1%-selective dotted lookup measured about 0.13 ms with shadow lookup plus a small temp sort, while forcing the no-sort correlated path was about 1.10 ms. SQLite's lookup-first plan is correct for that shape.
- Removed avoidable repeated JavaScript walks from explicit-null row decoding, Pointer distinct conversion, aggregate lookup field collection, aggregate multiply compilation, and index-shape checks.
- Validation: source/spec lint and build green; 97 adapter specs and 11 live Parse query integration specs green; regenerated standalone copy booted and completed indexed CRUD from an isolated temporary folder.
- Final hot-path check found indexed Array predicates rereading and parsing `_SCHEMA` despite the normal class schema already being cached. Normal queries now use the cached index declarations and only transactional/uncached paths read storage. A 20,000-iteration planner benchmark dropped from about 0.00474 ms to 0.00182 ms per WHERE build (about 2.6x); adapter coverage is 98/98.
- A detached-package shutdown probe found Node re-referenced the scheduler's receive `MessagePort` when its listener was attached, despite an earlier `unref()`. The port is now referenced only while resolvers are queued and unreferenced as soon as the O(1) queue drains. Indexed standalone CRUD still passes, and the detached process now exits naturally in about 0.46 seconds.

### Native Spatial Indexes
- Confirmed the bundled SQLite has both `RTREE` and `GEOPOLY`; GeoPoint and Polygon fields now use trigger-maintained R*Tree shadow indexes after an explicit geo index or the first selective geo query.
- Spatial rows use a hidden stable `_spatialId`, not SQLite `rowid`, so `VACUUM`, imported rows, updates, deletes, and table rebuilds keep the right object attached to each R*Tree row. The hidden column and helper index never appear in Parse rows or `getIndexes()` and are removed when the last geo field is deleted.
- Selective radius, box, point-in-polygon, and polygon-contains-point queries use R*Tree candidates and native GEOPOLY checks. Broad radius scans keep the compact distance UDF, broad polygons use GEOPOLY, broad boxes use native JSON extraction, and unbounded near queries still scan because SQLite has no spatial KNN operator.
- Lazy spatial DDL is disabled while a separate transaction connection owns the write lock; that query uses the correct scan path, and a later normal query may build the index without deadlocking.
- On 100,000 points, a small-radius lookup measured about 0.008 ms through R*Tree versus about 50 ms through the old JS distance scan. A selective polygon measured about 0.015 ms versus about 103 ms. Full-set GEOPOLY was about 42 ms versus about 116 ms through JS ray casting.
- Maintaining an active geo index added about 37 microseconds per sequential adapter write in a 20,000-write local run. Classes without an activated geo index do not pay that trigger cost.
- Class-scoped physical authData index names are now decoded correctly on duplicate writes, so batch signup races return Parse authData duplicate code 208 instead of falling through to the email duplicate probe.
- Validation: source lint and build green; 102 adapter specs, 35 GeoPoint specs, 13 applicable Polygon specs, 11 live query integration specs, and 8 authData uniqueness specs green. The generated package completed indexed spatial CRUD from an isolated temporary folder.
- Spatial capability detection now fails closed if an embedded SQLite omits compile-option diagnostics. Missing `RTREE` or `GEOPOLY` disables spatial artifacts and keeps geo queries on the existing scan path instead of breaking adapter startup or index creation; the forced-no-extension regression is green and adapter coverage is 103/103.
- Capability flags are independent: GeoPoint indexing needs only `RTREE`; Polygon indexing needs `RTREE` plus `GEOPOLY`; non-indexed polygon checks may still use `GEOPOLY` alone. The RTree-without-GEOPOLY regression is green and adapter coverage is 104/104.

### SQLite Provider Boundary Check
- Node 22.22.0 exposes experimental `node:sqlite` with synchronous `DatabaseSync` / `StatementSync`. Its bundled SQLite is 3.50.4 here and has JSONB, FTS5, RTREE, GEOPOLY, and JS scalar-function registration.
- The adapter's hot database surface is already small and shared by better-sqlite3 and node:sqlite: `prepare().run/get/all`, `exec`, `function`, and `close`. Provider-specific work is mainly construction, PRAGMA convenience, result/error cleanup, native-addon discovery, and backup handling.
- Five-round 50,000-row local median: better-sqlite3 versus node:sqlite was 60.52 vs 58.82 ms for transactional inserts, 35.59 vs 47.72 ms for 50,000 indexed gets, 194.64 vs 199.45 ms for JSON scans, and 217.72 vs 228.99 ms for JS-UDF scans.
- A small synchronous driver contract is reasonable and should keep the current adapter SQL unchanged. Do not force future expo-sqlite through that synchronous contract: share the SQL/schema planner, but give an Expo adapter a genuinely async executor so React Native's JS thread is not blocked.

### Async Execution Direction
- Keep the existing in-process synchronous engine as the lowest-latency default and put its database construction behind a narrow provider factory.
- Dogfood non-blocking Node operation by running the complete synchronous adapter engine in one dedicated worker thread. RPC must route transaction tokens to worker-owned connections, propagate schema-watch notifications, preserve errors, and avoid transferring database handles.
- Do not pretend a worker facade makes async SQL clients synchronous. Future providers such as expo-sqlite and libSQL need an async executor over shared query/schema planning; keeping that executor separate prevents Promise overhead from entering the direct better-sqlite3 hot path.

### Worker Execution And Provider Boundary
- Added opt-in `executionMode=worker`. One dedicated Node worker owns the complete adapter and its native handles; transaction tokens remain worker-local, unrelated work waits behind an active transaction, and schema notifications and Parse errors cross the RPC boundary.
- RPC bookkeeping is O(1): pending calls use a `Map`, and regular/transaction queues use head indices with amortized compaction. Idle workers are unreferenced after listener registration, referenced only while RPCs are pending, and no longer keep detached Node processes alive.
- Duplicate-key and sanitized-regex diagnostics are emitted through the host logger, while cloneable non-Error throws retain their original value. Mutable `disableIndexFieldValidation` updates are delivered in message order before subsequent calls.
- Added a typed synchronous provider contract and startup validation. The unchanged SQL engine runs with `better-sqlite3`, Node's `DatabaseSync`, or a custom synchronous provider implementing `prepare().run/get/all`, `exec`, function registration, and `close`.
- Worker mode deliberately skips the direct engine's event-loop scheduling hop because RPC already yields and SQLite executes off the main thread. A 3,000-write benchmark measured about 33.28 microseconds per direct create and 53.59 microseconds per worker create; a 3,000-row residual-regex query measured 11.18 versus 17.58 ms, while main-thread maximum delay fell from 10.99 to 1.37 ms.
- Promise-based SQL clients are not falsely accepted. The current engine has roughly 130 synchronous, result-dependent database touchpoints and relies on registered UDFs; expo-sqlite/libSQL require a separate async executor over extracted planning and capability fallbacks to avoid adding Promise overhead to the direct hot path.
- Validation: 109/109 adapter specs green; 99/99 executed schema specs green in worker mode; 264/264 executed vulnerability specs green in worker mode; generated standalone package completed worker CRUD with both providers from an isolated folder and an abandoned idle worker exited naturally in 0.16 seconds.
- A first full worker-mode Parse run executed 4,226 specs in 15 minutes. Its adapter-boundary failures were reproduced and fixed; remaining observed lifecycle/schema-cache failures explicitly selected MongoDB and could not connect because no local Mongo service was running.

### Final Async Provider Audit
- Worker support is loaded lazily, so direct mode and custom-provider imports do not load Node `worker_threads`. The synchronous provider boundary remains isolated from the RPC transport.
