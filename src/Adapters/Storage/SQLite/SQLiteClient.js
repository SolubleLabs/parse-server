...
const parseJSONContainer = (value: any, fallbackContainer: any) => {
  try {
...
const createMissingNestedContainer = (nextComponent: ?string) =>
  isNumericArrayIndexComponent(nextComponent) ? [] : {};

const getContainerEntry = (container: any, component: string) => {
...
const setContainerEntry = (container: any, component: string, value: any) => {
  if (Array.isArray(container) && isNumericArrayIndexComponent(component)) {
...
const removeContainerEntry = (container: any, component: string) => {
  if (Array.isArray(container) && isNumericArrayIndexComponent(component)) {
    const index = Number(component);
...
const ensureNestedContainer = (container: any, component: string, nextComponent: ?string) => {
...
const applyDynamicPathMutation = (
  rootContainer: any,
  ...
  for (let index = 0; index < pathComponents.length - 1; index += 1) {
    currentContainer = ensureNestedContainer(
      currentContainer,
      pathComponents[index],
      pathComponents[index + 1]
    );
  }
  ...
  switch (operation) {
    ...
    case 'Increment': {
      ...
      setContainerEntry(
        currentContainer,
        targetComponent,
        (Number.isFinite(baseValue) ? baseValue : 0) + (Number.isFinite(amount) ? amount : 0)
      );
      return rootContainer;
    }
    case 'Add': {
    ...
    case 'Remove': {
      ...
      setContainerEntry(
        currentContainer,
        targetComponent,
        targetArray.filter(item => !removeSet.has(canonicalJSONStringify(item)))
      );
      return rootContainer;
    }
    case 'Set':
...
function createClient(options: Object) {
  ...
  db.function(
    ...
    (targetStr, pathStr, operation, valueStr) => {
      ...
      const rootContainer = parseJSONContainer(
        targetStr,
        isNumericArrayIndexComponent(pathComponents && pathComponents[0]) ? [] : {}
      );
      const nextValue = applyDynamicPathMutation(
        rootContainer,
        Array.isArray(pathComponents) ? pathComponents : [],
        String(operation),
        parseJSONValue(valueStr)
      );
